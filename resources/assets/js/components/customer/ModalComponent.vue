         ...
         <form class="form-horizontal form-label-left" @submit.prevent="onSubmit">
                ...
                <div class="col-md-12 center-margin">
                    ...
                    <div class="col-md-12 center-margin">
                        <div class="form-group">
                            <label>Di động</label>
                            <input  class="form-control" v-model="formData.mobile" name="mobile" :disabled="action === 'view'" />
...
<!--                            <select class="form-control" v-model="formData.is_tester" name="is_tester" required :disabled="action === 'view'" >-->
...
export default {
    ...
    watch: {
        ...
        passCustomer: function(value) {
            ...
            // this.formData.is_tester = value.is_tester;
            this.customer          = value;
        }
    },
    methods: {
        onSubmit: function() {
            ...
            switch(that.action) {
                case 'create':
                    axios.post('/admin/api/customer', that.formData)
                        .then(function(res) {
                                swal({
                                    title: 'Thành công!',
                                    html: `Bạn đã tạo khách hàng <b>${that.formData.fullname}</b> thành công!`,
                                    icon: 'success'
                                });
                    ...
                    break;
                case 'edit':
                    axios.put(`/admin/api/customer/${that.customer.id}`, that.formData)
                    .then(function(res) {
                        swal({
                                title: 'Thành công!',
                                html: `Bạn đã sửa khách hàng <b>${that.formData.fullname}</b> thành công!`,
                                icon: 'success'
                            });
                            that.$emit('updateList', res.data.items);
                        $('#lessonModal').modal('hide');
                ...
                default:
                    swal({
                        ...
                        html: `Bạn sẽ xoá khách hàng <i>${that.formData.fullname}</i>`,
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    })
